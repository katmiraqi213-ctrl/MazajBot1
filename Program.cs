...
namespace MazajBot
{
    public class Program
    {
            ...
            _client.Messaging.OnMessage += async (client, message) =>
            {
                try
                {
                    ...
                    if (TryParseNumber(text, out int directNumber))
                    {
                        ...
                        // قبل اللعبة يتم تجاهل الرقم بصمت
                        return;
                    }
            ...
            bool directNumber)
        {
            ...
            // نتأكد أن اللعبة نفسها ما تغيرت
            if (_game != game ||
                !game.Started)
            {
                return;
